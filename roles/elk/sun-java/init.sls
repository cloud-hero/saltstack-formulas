{%- from 'roles/elk/sun-java/settings.sls' import java with context %}
...
update-java-bin-symlink:
  file.symlink:
    - name: /usr/bin/java
    - target: {{ java.java_home }}/bin/java

update-javac-bin-symlink:
  file.symlink:
    - name: /usr/bin/javac
    - target: {{ java.java_home }}/bin/javac
